...
function ClientFlow({ goToAdmin }) {
  ...
  const handleBookingSubmit = async (e) => {
    ...
    const payload = {
      id: Date.now().toString(), // Adicionado ID para garantir que funciona offline
      nome: formData.name,
    ...
    try {
      ...
      // MODO INVISÍVEL (FALLBACK)
      // Se o fetch falhar (bloqueio do telemóvel), guardamos localmente SEM MOSTRAR ERRO AO CLIENTE.
      ...
      setStep(4); // Vai sempre para a página de sucesso!
    }
  };
...
function AdminDashboard({ isLoggedIn, setIsLoggedIn }) {
  ...
  const fetchAgendamentos = async () => {
    ...
    try {
      ...
      // Juntamos os dados do Supabase (se houver) com os dados offline e mostramos!
